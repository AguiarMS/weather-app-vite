...
export const NextDaysProps = (dataDays: getLocationFor5daysProps[]) => {
  if (dataDays.length !== 0) {
    ...
    const fiveDays = [...new Map(datesWithHours.map(item => [item['info'], item])).values()] 
    ...
    return (
      ...
      {fiveDays.map( item => (
         <div  key={item.date.dt_txt}>
          <p className="px-1 font-bold text-white mb-2" >
          {handleTime(item.date.dt).convertData}</p>
          <p className="px-1 font-bold text-white mb-2" >
          {handleTime(item.date.dt).week}</p>
          <p className="px-1 font-bold text-white mb-2">
          temperatura {item.date.main.temp}
          </p>
          </div>
      ))}
      </div>
